...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case CAP_MOD:
            ...
            return false; // No output when pressed alone
        case KC_J:
            if (record->event.pressed && caps_held) {
                tap_code(KC_DOWN);
                return false; // Prevent 'J'
            }
            return true; // Normal J otherwise
        case KC_K:
            if (record->event.pressed && caps_held) {
                tap_code(KC_UP);
                return false; // Prevent 'K'
            }
            return true; // Normal J otherwise
        case KC_H:
            if (record->event.pressed && caps_held) {
                tap_code(KC_LEFT);
                return false; // Prevent 'H'
            }
            return true; // Normal H otherwise
        case KC_L:
            if (record->event.pressed && caps_held) {
                tap_code(KC_RIGHT);
                return false; // Prevent 'L'
            }
            return true; // Normal L otherwise
    }
    return true;
}
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_ansi_82(
        ...
        CAP_MOD,  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,              KC_ENT,             KC_HOME,
